...
switch (_mntCheck > _cost_Avrg) do{
	case true:
	{
		if (fuel vehicle player < 0.5) then {
		...
		}else{systemChat "Dispones mas del 50% de gasolina en el tanque";};
		sleep 3;
		if (damage vehicle player != 0) then {
			if ((_veh isKindOf "landvehicle") && (driver _veh == player)) exitWith { 
					...
					_Total_Fnds = _cost_Avrg / 2;
					[player, -_Total_Fnds] call HALs_money_fnc_addFunds;
					systemChat format ["Cost for repairing service: $%1",_Total_Fnds];
			};
		}else{systemChat "No tienes nada que reparar";};
	};
